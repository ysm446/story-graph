...
- [x] **2026-07-26 の作業**(上記以外。詳細は [changelog](../changelog.md)):
...
- [ ] **location の引き継ぎ**(2026-07-26 発案。小さい・効果あり): ノードの `location` が空欄なら、
  親から遡って直近の非空の場所を使う(実効ロケーション)。「場所が変わらない限り書かない」という
  自然な書き方を許すため。現状は空欄だと清書プロンプトが「場所: 不明」になり情報が落ちる。
  差し替え先は 清書プロンプト / 生成コンテキストの直近シーン / 相談チャットの `get_beats` /
  ノードカードの `@場所` 表示(引き継ぎ分は淡い色で出す)の 4 箇所。
  詳細は [node-inheritance.md](../design/node-inheritance.md) §2, §7
