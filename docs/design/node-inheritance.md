...
| `pos_x` / `pos_y` | キャンバス上の配置。NULL なら自動レイアウトだが、画面に出て高さが測れた時点でその位置を焼き付ける(`POST /layout/positions`)ので、実質すべてのノードが値を持つ。構造が変わってもノードを勝手に動かさないための仕組み。⟲(選択なし)で NULL に戻して自動レイアウトへ |
...
|---|---|---|---|
| ビート本文・題・場所・時期などの編集 | 変わらない(プロンプト文面のみ) | — | **自シーンだけ** `stale`(2026-08-01 フェーズ A で下流へ波及しなくなった) |
| cast の編集 | **登場が変わる**(導出)。ただし再 fold して**状態が同じなら下流へ波及しない**(early cutoff。並べ替えなど) | — | 自シーンは `stale`、下流は状態が変わったときだけ |
...
編集の波及粒度の設計背景は [chapters.md](chapters.md) §4(フェーズ A)。
イベント編集で early cutoff が効かないのは、置換のたびにイベント ID が再発行され
状態ハッシュが必ず変わるため(既知課題。ID 引き継ぎはフェーズ C の前提作業で入れる)。
...
- **location の引き継ぎ**: 空欄なら親から遡って直近の場所を使う(実効ロケーション)。
  ...
  ノード表示の 4 箇所を差し替える想定。
  → **場所を登録制エンティティにする設計に統合した**([places.md](places.md))。
  場所は state に `places[id].facts` として積み上がる(記憶・関係は持たない)ので、
  §1 の「引き継ぐもの」に 1 行増え、§2 の `location` は「引き継ぐ(実効ロケーション)」へ移る
